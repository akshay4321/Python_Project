...
def database_creation(db_file):
    ...
    try:
        ...
        cur.execute(
           "CREATE TABLE IF NOT EXISTS Booking (booking_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,hotel_id INTEGER NOT NULL,package_id INTEGER NOT NULL,customer_id INTEGER NOT NULL,from_date DATE NOT NULL,to_date DATE NOT NULL,total_amount FLOAT NOT NULL,createdate_time DATETIME NOT NULL,update_datetime DATETIME NOT NULL,delete_status INTEGER NOT NULL, CONSTRAINT FK_BookingHotel FOREIGN KEY (hotel_id) REFERENCES Hotel(hotel_id), CONSTRAINT FK_CustomerBooking FOREIGN KEY (customer_id) REFERENCES Customer(customer_id), CONSTRAINT FK_PackageBooking FOREIGN KEY (package_id) REFERENCES Package(package_id))")
